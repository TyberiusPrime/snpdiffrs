...
impl PreProcessRunner {
        ...
        let output_directories = samples_and_input_filenames
            ...
            .map(|(sample_name, _input_files)| {
                    let od = output_dir.join(format!(
                                ...
                                sample_name,  config.block_size, config.quality_threshold, 
    ...
    fn run(self) {
        let todo: Vec<(
            Arc<Vec<PathBuf>>, 
            &Chunk,
            PathBuf)> = self.chunks.iter().map(|chunk| {
            self.input_filenames.iter().zip(self.output_directories.iter()).map(move |(inputs, od)| {
                (inputs.clone(), chunk, od.join(format!("{}_{}.snpdiffrs_block", chunk.chr, chunk.start).to_owned()))
            })}).flatten().collect();

        let _: Vec<()> = todo.par_iter().map(|(input_filenames, chunk, output_filename)| {
            if !output_filename.exists() {
                let cov = Coverage::from_bams(&input_filenames.as_ref().iter().map(|x| x.as_path()).collect::<Vec<_>>(), 
                    chunk.tid, chunk.start, chunk.stop, self.quality_threshold, &self.filter_homo_polymer_threshold);

            //lz4_hc::compress_to_vec(data, &mut comp, lz4_hc::CLEVEL_DEFAULT)?;
            let mut fh = File::create(output_filename.clone()).expect(&format!("Unable to create file {:?}", output_filename));
            let bincoded = bincode::serialize(&cov).expect("bincoding failed?");
            let mut comp = Vec::new();
            lz4::compress_to_vec(&bincoded, &mut comp, lz4::ACC_LEVEL_DEFAULT).expect("Compression failed");
            fh.write_all(&comp).expect("Could not write all data")
            }
        }).collect();
